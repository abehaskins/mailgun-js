...
class Request {
    ...
    _request(method, resource, data, fn) {
        ...
        if (this.retry > 1) {
            async_1.retry(this.retry, (retryCb) => {
    ...
    handleMimeObject(key, obj) {
        ...
        else if (obj instanceof stream_1.Readable) {
    ...
    handleAttachmentObject(key, obj) {
        ...
        else if (typeof obj === "object" && obj instanceof attachment_1.Attachment) {
    ...
    handleResponse(res) {
        ...
        res.on("end", () => {
            ...
            const unsafeRes = res;
            const skipContentTypeCheck = unsafeRes.req &&
                unsafeRes.req.path &&
